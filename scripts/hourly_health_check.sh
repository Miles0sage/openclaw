...
LOG="/root/openclaw/data/health_check.log"
...
if [ "$DISK_USAGE" -gt 90 ]; then
    ...
    find /root/openclaw/data/jobs/worktrees/ -maxdepth 1 -mtime +3 -type d -exec rm -rf {} + 2>/dev/null
    # Clean old log files
    find /root/openclaw/data/ -name "*.log" -mtime +7 -delete 2>/dev/null
