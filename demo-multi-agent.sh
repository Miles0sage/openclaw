...
CODER_START=$(date +%s)
CODER=$(timeout 45 curl -s http://152.53.55.207:11434/api/generate -d @/tmp/coder.json | jq -r '.response')
CODER_END=$(date +%s)
...
SECURITY=$(timeout 45 curl -s http://152.53.55.207:11434/api/generate -d @/tmp/security.json | jq -r '.response')
SECURITY_END=$(date +%s)
