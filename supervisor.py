...
def create_pr_for_job(job_id: str, project_root: str, sub_tasks: list, summary: str) -> Optional[str]:
    """
    Create a GitHub PR for a completed complex job.
    ...
    try:
        ...
        if not status.stdout.strip():
            logger.info(f"Job {job_id}: No git changes to PR")
            return None
        ...
        descriptions = "\n".join(f"- {st.get('description', '')[:100]}" for st in sub_tasks)
        commit_msg = f"agent({job_id}): {summary[:100]}\n\nSub-tasks:\n{descriptions}"
        ...
        pr_body = f"## Agent Job: {job_id}\n\n{summary}\n\n### Sub-tasks\n{descriptions}\n\n---\n*Auto-generated by OpenClaw Supervisor*"
        pr_result = subprocess.run(
            ["gh", "pr", "create",
             "--title", f"[Agent] {summary[:60]}",
             "--body", pr_body,
             "--base", "main"],
            capture_output=True, text=True, cwd=project_root, timeout=30
        )
        ...
        if pr_result.returncode == 0:
            ...
            logger.info(f"Job {job_id}: PR created → {pr_url}")
            # Switch back to main
            subprocess.run(["git", "checkout", "main"], capture_output=True, text=True, cwd=project_root, timeout=10)
            return pr_url
        else:
            logger.warning(f"Job {job_id}: PR creation failed — {pr_result.stderr}")
            subprocess.run(["git", "checkout", "main"], capture_output=True, text=True, cwd=project_root, timeout=10)
            return None

    except Exception as e:
        logger.error(f"Job {job_id}: PR creation error — {e}")
...
async def maybe_decompose_and_execute(job: dict, project_root: str = "/root/openclaw") -> Optional[dict]:
    ...
    while time.time() - start_time < SUB_TASK_TIMEOUT:
        ...
        for sr in spawned:
            if sr["job_id"] in completed:
                continue
            # Check completion marker file (written by agent script before sleep)
            done_marker = Path(f"/root/openclaw/data/agent_outputs/heartbeats/done-{sr['job_id']}")
            agent_status = spawner.get_agent_status(sr["pane_id"])
            is_done = done_marker.exists() or (agent_status and agent_status.get("status") == "exited")
            if is_done:
                ...
                logger.info(f"Sub-task {sr['job_id']} completed ({runtime}s)")
            else:
                all_done = False
    ...
    # Create PR for successful complex jobs (if in a git repo with changes)
    if result["success"] and len(successful) >= 2:
        pr_url = create_pr_for_job(
            job_id=job_id,
