...
ExecStart=/usr/bin/python3 /root/openclaw/gateway.py
Restart=always
RestartSec=5
TimeoutStopSec=300
...
1. Point your domain (e.g., `gateway.overseerclaw.uk`) to your server IP
...
- `/root/openclaw/.opencode.json` (main project)
...
```bash
curl https://gateway.overseerclaw.uk/api/health
```
...
curl -H "Authorization: Bearer $GATEWAY_AUTH_TOKEN" \
  https://gateway.overseerclaw.uk/api/monitoring/active
```

### Dashboard Access

- **Job Viewer:** `https://gateway.overseerclaw.uk/job_viewer.html`
- **Mission Control:** `https://gateway.overseerclaw.uk/mission_control.html`
